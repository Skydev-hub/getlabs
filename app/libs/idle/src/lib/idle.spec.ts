  ...
  describe('without KeepaliveSvc integration', () => {
    ...
    describe('runtime config', () => {
      ...
      it('setIdleName() when expiry is not instance of LocalStorageExpiry should throw error', () => {
        ...
        }).toThrowError('Cannot set expiry key name because no LocalStorageExpiry has been provided.');
      });
    ...
    describe('watching', () => {
      ...
      it('watch() should set expiry', () => {
        ...
        expiry.mockNow = now;
        instance.watch();
        expect(expiry.last()).toEqual(new Date(now.getTime() + (instance.getIdle() + instance.getTimeout()) * 1000));
        instance.stop();
      });
